...
int write_some(RingBuffer *buffer, int fd, int is_socket)
{
    ...
    check(
        bfindreplace(data, &NL, &CRLF, 0) == BSTR_OK,
...
descriptor 0) and `socket`, which it uses to talk to a server. The
code uses `RingBuffers` to store the data and copy it around. You
can consider the functions `read_some` and `write_some` early
prototypes for similar functions in the `RingBuffer` library.

This little bit of code contains quite a few networking functions
that you may not know. As you hit a function that you don't know,
look it up in the man pages and make sure you understand it. This
one little file might inspire you to then research all of the APIs
required to write a little server in C.
*/
