...
static int test_search(RadixMap *map)
{
    ...
    for (i = map->end / 2; i < map->end; i++) {
        ...
        check(
            found != NULL, "Didn't find %u at %u.", d->data.key, i
        );
        check(
...
static char *test_operations()
{
    ...
    mu_assert(
        make_random(map), "Didn't make a random fake radix map."
    );
    ...
    while (map->end > 0) {
        ...
        mu_assert(
            RadixMap_delete(map, el) == 0, "Didn't delete it."
        );
        mu_assert(
            old_end - 1 == map->end, "Wrong size after delete."
        );
