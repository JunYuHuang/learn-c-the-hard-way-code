...
`String_setup_skip_chars` and `String_base_search`. These are then
used in the other functions to actually implement the searching
styles I want. Study these first two functions and compare them to
the Wikipedia description so that you know what's going on.
*/
...
/*
The `String_find` then just uses these two functions to do a find
and return the position found. It's very simple, and I'll use it to
see how this build skip chars phase impacts real, practical
performance. Keep in mind that you could maybe make this faster, but
I'm teaching you how to confirm theoretical speed after you
...
int String_find(bstring in, bstring what)
{
    ...
    found = String_base_search(
        haystack, hlen, needle, nlen, skip_chars
    );
...
static inline void StringScanner_set_needle(
    StringScanner *scan, bstring tofind
)
{
    scan->needle = (const unsigned char *)bdata(tofind);
    scan->nlen = blength(tofind);

    String_setup_skip_chars(
        scan->skip_chars, scan->needle, scan->nlen
    );
}
...
/*
The `StringScanner_scan` function then follows the common pattern I
use of create, scan, and destroy, and is used to incrementally scan
a string for another string. You'll see how this is used when I show
you the unit test that will test this out.
*/
