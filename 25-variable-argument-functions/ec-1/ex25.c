...
// wrapper for calling `fgets()`
int read_string(char **out_string, int max_buffer)
{
    // `**out_string`:
    // - is a pointer to a char pointer
    // - is like an array of strings (i.e. char arrays)

    // new string in memory
    *out_string = calloc(1, max_buffer + 1);
    check_mem(*out_string);

    // save user input as a string in the `out_string` array
...
// wrapper for calling `read_int()` but for ints
int read_int(int *out_int)
{
    ...
    // saves `input` to the int that `out_int` points to
    *out_int = atoi(input);
...
int read_scan(const char *fmt, ...)
{
    ...
    // `out_int`: a pointer to an int
    int *out_int = NULL;

    // `out_char`: a pointer to a char (i.e. like a string)
    char *out_char = NULL;

    // `out_string`:
    // - a pointer to a pointer to a char
    // - like an array of char arrays
            ...
            switch (fmt[i]) {
                ...
                case 'd':
                    ...
                    */
                    ...
                    // - sets `out_int` to the next var arg,
                    // an int pointer, from `argp`
                ...
                case 'c':
                    ...
                    // - sets `out_char` to the next var arg,
                    // a char pointer, from `argp`
                    out_char = va_arg(argp, char *);

                    // - sets the `out_char`-pointed char value to
                    // the next user-entered char
                ...
                */
                case 's':
                    ...
                    // - sets `out_string` to an array of strings
