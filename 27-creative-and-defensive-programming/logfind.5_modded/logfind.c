...
{
    debug(
        "use_or: %d, found_count: %d, search_len: %d",
...
)
{
    ...
    // read each line of the file and search that line for
    // the contents
    while (fgets(line, MAX_LINE - 1, file) != NULL) {
        for (i = 0; i < search_len; i++) {
            if (strcasestr(line, search_for[i]) != NULL) {
                debug(
                    "file: %s, line: %s, search: %s",
                    filename,
                    line,
                    search_for[i]
                );
...
{
    // check(*use_or == 1 || *use_or == 0, "Invalid default use_or.");
    (*argc)--;
    (*argv)++;
...
int main(int argc, char *argv[])
{
    ...
    check(
        parse_args(&use_or, &argc, &argv) == 0,
        "USAGE: logfind [-o] words"
    );

    check(
        list_files(&files_found) == 0,
