...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only)
{
    ...
    if (apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS) {
        ...
        apr_fnmatch(DEPEND_PAT, info.path, 0) == APR_SUCCESS
    ) {
        ...
        if (info.scheme) {
            ...
            rc = Shell_exec(
                CURL_SH, "URL", url, "TARGET", depends_file, NULL
            );
        ...
        apr_fnmatch(TAR_GZ_PAT, info.path, 0) == APR_SUCCESS
    ) {
        if (info.scheme) {
            rc = Shell_exec(
                CURL_SH, "URL", url, "TARGET", TAR_GZ_SRC, NULL
            );
        ...
        apr_fnmatch(TAR_BZ2_PAT, info.path, 0) == APR_SUCCESS
    ) {
        if (info.scheme) {
            rc = Shell_exec(
                CURL_SH, "URL", url, "TARGET", TAR_BZ2_SRC, NULL
            );
            check(rc == 0, "Curl failed.");
        }
...
)
{
    ...
    if (rc == 1) {
        rc = Command_build(
            p, url, configure_opts, make_opts, install_opts
        );
